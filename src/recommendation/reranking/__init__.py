...
look up business/catalog data itself. Eligibility is a HARD pre-retrieval
gate applied before this package ever sees a candidate (every id
reaching `rerank` is already eligible), plus a final lightweight
re-validation applied after it (docs/data-mapping.md section 5) -
`rerank` itself is unaffected either way, it never filters.
"""
