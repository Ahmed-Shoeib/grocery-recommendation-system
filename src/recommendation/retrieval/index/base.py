...
Every backend (FAISS now, ScaNN in Phase 10) does normalized-inner-product
search over the L2-normalized 128-D Two-Tower item embeddings produced in
Phase 4 - since both query and item vectors are unit-norm, inner product
and cosine similarity are the same quantity, so `search` returns cosine
similarity scores regardless of backend.
"""
