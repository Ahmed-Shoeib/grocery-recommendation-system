...
@pytest.fixture
def scenario():
    # 12 in-stock products across 4 categories, plus 2 with is_active=False
    # (production-safe contract: is_active no longer gates eligibility, so
    # 200 is genuinely eligible now - only stock_quantity does) plus one
    # explicitly out-of-stock (201) to exercise eligibility.
...
def test_sparse_history_user_blends_sources(scenario):
    profile = EngagementProfile(
        user_id=2, profile=UserProfile(user_id=2, preferred_categories=["Cat2"]),
...
def test_no_history_user_uses_fallback_only(scenario):
    profile = EngagementProfile(user_id=3, profile=UserProfile(user_id=3, preferred_categories=["Cat1"]))
    result = _run(scenario, profile)
...
def test_eligibility_excludes_only_out_of_stock(scenario):
    """Production-safe contract (docs/production-feature-parity-audit.md):
    `is_active` no longer gates eligibility (no real `isActive` column in
    production) - only 201 (out of stock) is excluded by the HARD
    PRE-RETRIEVAL gate; 200 (is_active=False but in stock) is now
    genuinely eligible. Excluded products never reach candidate
    generation at all, so the final-validation exclusion count stays 0.
    """
    ...
    assert result.num_excluded_pre_retrieval == 1
    assert result.num_excluded_by_eligibility == 0


def test_pre_retrieval_gate_excludes_only_out_of_stock_from_candidate_generation(scenario):
    """Requirement: an out-of-stock product must not PARTICIPATE in
    ...
    just the final Top-N. `is_active=False` no longer excludes (see
    docs/production-feature-parity-audit.md) - 200 legitimately
    participates.
    """
    profile = EngagementProfile(
...
def test_fallback_no_history_never_surfaces_out_of_stock(scenario):
    ...
    retrieval - deliberately picks the preferred category ("Cat1") that
    contains the out-of-stock product (201), to actually exercise the
    fallback ranking functions with it present.
    """
    profile2 = EngagementProfile(user_id=15, profile=UserProfile(user_id=15, preferred_categories=["Cat1"]))
    result2 = _run(scenario, profile2, top_n=20)
    assert result2.tier.value == "no_history"
...
def test_eligible_products_retrieve_normally(scenario):
    """Sanity check: pre-retrieval eligibility only excludes the one
    out-of-stock product - every in-stock product (including the
    is_active=False one, which is production-safe-eligible) is a normal,
    ...
    result = _run(scenario, profile, top_n=20)
    assert result.product_ids
    assert set(result.product_ids) <= {100 + i for i in range(12)} | {200}
    for pid in result.product_ids:
        pf = scenario["product_features"][pid]
        assert pf.stock_quantity > 0


def test_fill_rate_drops_when_fewer_eligible_products_than_requested(scenario):
    """13 eligible products exist in the catalog (12 base + 200, which is
    production-safe-eligible despite is_active=False); requesting more
    than that must honestly return fewer than requested, not pad/crash.
    """
    profile = EngagementProfile(
    ...
    result = _run(scenario, profile, top_n=20)
    assert len(result.product_ids) == 13
    assert result.fill_rate == pytest.approx(13 / 20)
    assert result.num_excluded_pre_retrieval == 1
...
def test_stock_and_active_changes_do_not_require_index_or_model_rebuild(scenario):
    ...
    assert len(result_after.product_ids) == 12
...
def test_final_validation_catches_product_that_becomes_unavailable_after_retrieval(scenario):
    ...
    # Production-safe contract: stock_quantity is the real availability
    # signal (is_active no longer gates eligibility at all) - see
    # docs/production-feature-parity-audit.md.
    fresher_features = dict(scenario["product_features"])
    fresher_features[100] = dataclasses.replace(fresher_features[100], stock_quantity=0)
    ...
    assert "in_stock" in result.excluded_reasons.get(100, [])
...
def test_no_eligible_products_returns_empty_result_gracefully(scenario):
    ...
    user_features = build_user_features(profile, scenario["product_lookup"], scenario["product_embeddings"], FeatureConfig())
    # Production-safe contract: stock_quantity is what actually gates
    # eligibility now (docs/production-feature-parity-audit.md).
    all_ineligible = {pid: dataclasses.replace(pf, stock_quantity=0) for pid, pf in scenario["product_features"].items()}
...
def test_result_has_no_duplicate_product_ids(scenario):
    profile = EngagementProfile(
        user_id=6, profile=UserProfile(user_id=6, preferred_categories=["Cat0"]),
