...
def test_is_active_no_longer_gates_eligibility():
    """Production-safe contract (docs/production-feature-parity-audit.md):
    the real SQL Server `Products` table has no `isActive` column at all,
    so `is_active` was removed from the eligibility rule set entirely - a
    product with `is_active=False` but in stock must still be eligible.
    """
    rules = build_eligibility_rules(EligibilityConfig(require_in_stock=True))
    result = apply_eligibility([1], {1: _pf(1, is_active=False)}, rules)
    assert result.eligible_ids == [1]
    assert result.excluded_ids == []
    assert "is_active" not in [r.name for r in rules]
...
def test_inactive_and_out_of_stock_excluded_only_for_stock():
    rules = build_eligibility_rules(EligibilityConfig(require_in_stock=True))
...
def test_require_in_stock_false_allows_zero_stock():
    rules = build_eligibility_rules(EligibilityConfig(require_in_stock=False))
    result = apply_eligibility([1], {1: _pf(1, stock_quantity=0)}, rules)
    assert result.eligible_ids == [1]
...
def test_preserves_candidate_order_within_eligible_and_excluded():
    rules = build_eligibility_rules(EligibilityConfig(require_in_stock=True))
    features = {1: _pf(1), 2: _pf(2, stock_quantity=0), 3: _pf(3)}
